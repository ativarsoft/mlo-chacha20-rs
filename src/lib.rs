...
impl ChaCha20 {
    ...
    pub fn key_setup(&mut self, k: &[u8], k_bits: usize, _iv_bits: usize) {
        ...
        let constants: [u8; 32] = {
            if k_bits == 256 {
                k_index += 16;
                self.string_to_key(SIGMA)
            } else {
                self.string_to_key(TAU)
            }
        };
        let k_slice = &k[k_index + (4 * 0) .. k_index + (4 * 1)];
    ...
    pub fn test1(malloc: Allocator) -> isize {
        ...
        if c != *&TEST1_EXPECTED_CYPHERTEXT {
            return 1;
        }
        return 0;
    }

    pub fn test2(malloc: Allocator) -> isize {
        let mut key: [u8; 32] = [0; 32];
        key[31] = 1;
        ...
        if c[0 .. plaintext.len()] != expected_cyphertext {
            return 2;
        }
        return 0;
    }

    pub fn test3(malloc: Allocator) -> isize {
...
#[no_mangle]
extern "C"
fn chacha20_test1(allocator: Allocator) -> isize {
    return ChaCha20::test1(allocator);
}

#[no_mangle]
extern "C"
fn chacha20_test2(allocator: Allocator) -> isize {
    return ChaCha20::test2(allocator);
}

#[no_mangle]
extern "C"
fn chacha20_test3(allocator: Allocator) -> isize {
    return ChaCha20::test3(allocator);
}
...
pub mod tests {
    ...
    #[test]
    fn test1() {
        let rc: isize = ChaCha20::test1(dummy_allocator);
        assert_eq!(rc, 0);
    }

    #[test]
    fn test2() {
        let rc: isize = ChaCha20::test2(malloc);
        assert_eq!(rc, 0);
    }

    #[test]
    fn test3() {
        let rc: isize = ChaCha20::test3(malloc);
        assert_eq!(rc, 0);
    }
